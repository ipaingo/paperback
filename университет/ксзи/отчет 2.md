...
## метод решения
Задача решается написанием схемы Эль-Гамаля на языке C#. Для генерации псевдопростых чисел используется тест Миллера-Рабина. Для нахождения первообразного корня используется факторизация числа Эйлера от простого числа.
...
2. Выбирается случайное число $! < k < p-1$, взаимно простое с $p-1$, вычисляется $r = g^k \mod p$.
